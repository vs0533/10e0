...
/// <summary>
/// PostgresOutboxLock 单元测试 — Happy Path（issue #81 / #74 子任务）
///
/// <para>
/// <b>测试范围</b>：本文件只验证 PostgresOutboxLock 在 LINQ 路径上的契约行为
/// （assertion：LockedUntil / LockedByInstance 写入、Release 清空、所有权校验、
/// 锁过期可抢占、缺失消息返回 false）。
/// </para>
///
/// <para>
/// <b>为什么用 InMemory 而非真实 PostgreSQL</b>：
/// 真实 PostgreSQL provider（<c>Npgsql.EntityFrameworkCore.PostgreSQL</c>）不在测试项目
/// 依赖中，且 PostgreSQL 行级锁（<c>FOR UPDATE SKIP LOCKED</c>）需要真实数据库实例才能验证；
/// issue 明确"单元测试用 InMemory provider 跑 happy path；并发安全测试放到 (3)"。
/// InMemory 下 <c>FOR UPDATE</c> / <c>SKIP LOCKED</c> 是 noop，<c>UPDATE</c> 实际执行，
/// 因此断言"Lock 字段被写入 / 清空"即等同验证 LINQ 路径的行为契约。
/// </para>
///
/// <para>
/// <b>为什么独立文件而非合并到 SqlServerOutboxLockTests</b>：
/// 1. 两个实现在生产代码里是独立类（不同 SQL 语法、不同 identifier quoting），测试也应对偶独立；
/// 2. issue (3) 真实数据库集成测试时可直接追加 scenario 而不污染 acceptance 测试；
/// 3. SqlServer / Postgres 任一 provider 重构时定位测试范围更精准。
/// </para>
///
/// <para>
/// <b>不验证</b>：
/// - Postgres <c>UPDATE "OutboxMessages" SET ...</c> 的真实执行（SQL 路径走
///   <c>ExecuteSqlInterpolatedAsync</c>，InMemory 上不支持该 API）
/// - 跨进程 / 多实例并发抢占（属于 issue (3) 范畴，需 Postgres Testcontainers）
/// </para>
/// </summary>
[Trait("Category", "Unit")]
public sealed class PostgresOutboxLockTests
{
    // ================================================================
    // Test Infrastructure — 与 SqlServerOutboxLockTests 同款，自有副本
    // ================================================================
    ...
    [Fact]
    public async Task GivenPostgresLockAndSameInstanceReacquire_WhenTryAcquire_ThenSucceedsAsReentrant()
    {
        // Arrange — 同一实例再次 TryAcquire 已持有的行
        var dbName = Guid.NewGuid().ToString("N");
        var factory = CreateFactory(dbName);
        ...
        // Then — 拿得到且 LockedByInstance 不被错乱覆盖
        acquired.Should().BeTrue(
    ...
    [Fact]
    public async Task GivenPostgresLockAndMessageHeldByOtherInstance_WhenTryAcquire_ThenReturnsFalse()
    {
    ...
    [Fact]
    public async Task GivenPostgresLockAndExpiredLease_WhenTryAcquire_ThenSucceedsAndOverwritesHolder()
    {
    ...
    [Fact]
    public async Task GivenPostgresLockAndMissingMessage_WhenRelease_ThenDoesNotThrow()
    {
    ...
    [InlineData("")]
    [InlineData("   ")]
    public async Task GivenPostgresLockAndInvalidMessageId_WhenTryAcquire_ThenReturnsFalse(string? invalidId)
    {
