...
/// <summary>
/// SQL Server 行级锁 provider — 通过 <c>UPDATE</c> 设置 <c>LockedByInstance</c> / <c>LockedUntil</c>
/// 抢占 OutboxMessage 行的处理权。
///
/// <para>
/// <b>为什么是 <c>UPDATE</c> 而不是显式 <c>SELECT ... WITH (UPDLOCK, READPAST)</c>？</b>
/// 多实例 Relay 抢占同一行时，单条 <c>SELECT</c> 拿不到原子互斥；
/// 改用 <c>UPDATE ... WHERE (LockedByInstance IS NULL OR LockedUntil &lt;= now)</c>：
/// - <c>UPDLOCK</c> 等价语义天然由 <c>UPDATE</c> 排他锁提供
/// - <c>READPAST</c> 等价语义由 "未锁或锁已过期" 的 WHERE 条件实现
...
/// <see cref="OutboxAdminService{TContext}"/> / <see cref="OrgTreeService{TContext}"/>
/// 等所有 Outbox 相关服务保持一致的泛型形态；DI 阶段以 <c>SqlServerOutboxLock&lt;DemoDbContext&gt;</c>
/// 形式显式闭合。
/// </para>
///
/// <para>
...
/// 其他 provider（含 <c>Microsoft.EntityFrameworkCore.SqlServer</c>）走
/// <c>ExecuteSqlInterpolatedAsync</c> 拼 <c>UPDATE</c> 路径。两条路径的行为契约完全一致。
/// </para>
///
/// <para>
...
public sealed class SqlServerOutboxLock<TContext> : IOutboxLock
    where TContext : DbContext
{
    ...
    public SqlServerOutboxLock(IDbContextFactory<TContext> factory, TimeProvider? timeProvider = null)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        // 真实 SqlServer provider 路径：单条 UPDATE 用 WHERE 条件天然实现
        ...
        CancellationToken cancellationToken)
    {
        ...
        // 真实 SqlServer provider 路径：所有权校验通过 WHERE 子句实现
